...
export interface DocumentPanelProps {
  ...
  placeholder?: string;
  /**
   * How the mode's document renders: markdown through the shared renderer, or
   * a whole html-explainer file shown in a sandboxed iframe. Absent means
   * markdown, so a server that has not declared the field keeps today's panel.
   */
...
/**
 * The mode's document — markdown or a whole HTML file — read or edited, one
 * at a time.
 *
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-1.5">
        ...
        >
          {(["read", "edit"] as const).map((value) => (
            ...
            >
              {value === "read" ? (isHtml ? "Ver" : "Ler") : "Editar"}
            </button>
          ))}
        </div>
      ...
      {view === "edit" ? (
        ...
        // The document is generated by a language model, so it is not trusted
        // input: `allow-scripts` lets the html-explainer runtime (tabs, syntax
        // highlight, copy buttons) run, and the missing `allow-same-origin`
        // keeps the frame from touching this window — no origin, no storage,
        // no parent access. A fresh srcDoc reloads the frame on every change.
        <iframe
